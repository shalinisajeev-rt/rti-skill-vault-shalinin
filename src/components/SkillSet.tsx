...
// Placeholder skills for other tabs
const CICD_SKILLS = [
...
const SkillSet = ({ employees, setEmployees }: SkillSetProps) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isExcelImportOpen, setIsExcelImportOpen] = useState(false);
  const [editingEmployee, setEditingEmployee] = useState<Employee | null>(null);
  const [activeSkillTab, setActiveSkillTab] = useState('autosar-classic');
  const { toast } = useToast();

  const currentSkills = SKILL_CATEGORIES[activeSkillTab as keyof typeof SKILL_CATEGORIES] || AUTOSAR_CLASSIC_SKILLS;
  ...
  return (
    <div className="p-6">
      ...
      <div className="max-w-[95vw] mx-auto mt-6">
